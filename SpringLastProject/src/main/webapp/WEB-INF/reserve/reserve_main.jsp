                    			...
                    			<div style="overflow-y:auto; height: 500px;">
                    				...
                    				<tr>
                    					<td colspan="2" class="text-center">
                    						...
                    						<button class="btn-xs btn-success" @click="next">다음</button>
                    					</td>
                    				</tr>
                    				</table>
                    			</div>
...
<script>
	let reserveApp=Vue.createApp({
		...
		//vue클래스의 멤버변수/멤버메소드 : 사용시 반드시 this
		methods:{
			reserve(){
			...
			next(){
				this.curpage=this.curpage>1?this.totalpage>this.curpage+1:this.curpage
				this.dataRecv()
			},
			dataRecv(){
				//@getmapping, axios.post	= @postmapping
				//오류 : 404(파밀명, 파일찾기실패),405(get/post), 400(매개변수 오류), 500(서버 오류) 
				axios.get('../reserve/reserve_main_vue.do', {
					...
					this.totalpage=response.data.totalpege
					this.type=response.data.type
					// response.data{name:'', sex:''....}
					//response.data.name
	...
	}).mount("#reserveApp")	//reserveApp내의 html만 제어(section태그 내)
</script>
</body>
</html>