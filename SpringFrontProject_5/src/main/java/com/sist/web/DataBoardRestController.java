...
public class DataBoardRestController {
	...
	// C:\\Users\\sist113_1\\Documents\\workspace-sts-3.9.18.RELEASE\\SpringStudy2\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\SpringFrontProject_5\\upload
	...
	public String databoard_insert(DataBoardVO vo, HttpServletRequest request) {
		 ...
		 * System.out.println(vo.getFiles()); if(vo.getFiles().size()>0) { for(int i=0;
		 * i<vo.getFiles().size(); i++) {
		...
		try {
			...
			if(!dir.exists()) {		//업로드 폴더가 없을시 자동생성
				dir.mkdir();
			}
			path=path.replace("\\", File.separator);	//운영체제별 자동설정 : separator
	...
	public void databoard_download(String fn, HttpServletResponse response, HttpServletRequest request) {
		try {
			...
			int i=0;		//읽은 바이트 수
			byte[] buffer=new byte[1024];		//한번에 읽을 바이트 수
